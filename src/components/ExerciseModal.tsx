...
export default function ExerciseModal({ exercise, onClose, onSaved }: Props) {
    ...
    useEffect(() => {
        api('/api/exercises')
            .then((data: any[]) => {
                const focusList = [...new Map(data.map((ex: any) => [ex.focus_id, { id: ex.focus_id, name: ex.focus_name }])).values()] as Focus[];
    ...
    const handleSave = async () => {
        if (!name.trim()) { setError('El nombre del ejercicio es obligatorio.'); return; }
        setSaving(true);
        setError('');

        try {
            const payload = { name, description, focus_id: focusId, video_url: videoUrl || null };
        ...
        <div className={styles.overlay}>
            <div className={styles.modal}>
                <div className={styles.header}>
                    <h2 className={styles.title}>{exercise ? 'EDITAR EJERCICIO' : 'NUEVO EJERCICIO'}</h2>
                ...
                {loading ? (
                    ...
                    <form onSubmit={(e) => { e.preventDefault(); handleSave(); }}>
                        ...
                        <div className={styles.footer}>
                            {isEditing && (
                                <button type="button" onClick={handleDelete} className={styles.deleteBtn}><Trash2 size={16} /> ELIMINAR</button>
                            )}
